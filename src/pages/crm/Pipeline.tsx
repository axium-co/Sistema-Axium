...
const DRAGGABLE_TYPE = 'LEAD_CARD';
...
const LeadCardInner = memo<LeadCardInnerProps>(({ lead, isClosed, onWhatsAppClick }) => (
  <>
    <div className="font-bold text-[11px] md:text-[13px] text-black mb-0.5">{lead.name}</div>
    <div className="text-[9px] md:text-[10px] text-neutral-500 font-semibold mb-2 md:mb-3">
      {lead.niche}
    </div>
      ...
      <div className="pt-2 md:pt-3 border-t border-neutral-100 mt-2 md:mt-3">
        <div className="flex justify-between items-center gap-2">
          <div className="flex flex-col">
            <span className="text-[7px] md:text-[8px] text-neutral-400 font-bold uppercase tracking-[0.1em]">
              Valor
            </span>
            <span className="text-[10px] md:text-xs font-black text-black mt-0.5">
              {lead.value || 'R$ 0,00'}
            </span>
          </div>
    ...
    {!isClosed && (
      <div className="flex justify-end mt-2">
        <div className="w-6 h-6 rounded-full bg-neutral-50 flex items-center justify-center text-[10px] font-bold text-neutral-400 transition-colors">
          ↕
        </div>
      </div>
    )}
...
const CRMPipeline = () => {
  ...
  useEffect(() => {
    ...
    if (dirty) setOptimisticStages(next);
  }, [leads]);

  const updateLeadRef = useRef(updateLead);
  useEffect(() => { updateLeadRef.current = updateLead; }, [updateLead]);

  // ─── Leads com optimistic stages aplicados ────────────────
  const effectiveLeads = useMemo(() => {
    return (leads || []).map((lead) => ({
      ...lead,
      stage: optimisticStages[lead.id] || lead.stage,
    }));
  }, [leads, optimisticStages]);

  const filteredLeads = useMemo(() => {
    return applyFilters(effectiveLeads, filters);
  ...
  const handleDragEnd = useCallback(
    (result: DropResult) => {
      ...
      updateLeadRef
        ...
        .then(() => {
          console.log(`[Pipeline] Lead ${draggableId} movido para "${newStage}"`);
        })
        .catch((err: unknown) => {
          console.error('[Pipeline] Erro ao salvar movimentação:', err);
      ...
      <div className="mb-4 flex justify-between items-start">
        <div>
          ...
          <p className="text-neutral-500 text-sm">
            Arraste e solte os cards para mover entre etapas. Alterações salvam automaticamente.
          </p>
        </div>
        <div className="relative group">
          ...
          >
            ...
            {hasActiveFilters && <span className="absolute top-1 right-1 w-1.5 h-1.5 bg-red-500 rounded-full" />}
          </button>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-x-auto pb-6">
          <div className="flex gap-5 min-w-max h-full items-start">
            {columnData.map(({ stage, leads: columnLeads, count, totalValue }) => {
              ...
              return (
                <Droppable droppableId={stage} key={stage} type={DRAGGABLE_TYPE}>
                  {(provided, snapshot) => (
                    <div
                      ref={provided.innerRef}
                      {...provided.droppableProps}
                      className={`bg-neutral-50/50 border-2 rounded-md w-[300px] shrink-0 flex flex-col max-h-full transition-colors ${
